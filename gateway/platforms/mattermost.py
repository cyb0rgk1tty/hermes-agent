...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        # Build media URLs for any attachments.
        media_urls: List[str] = []
        media_types: List[str] = []
        for fid in file_ids:
            # Mattermost file download URL (requires auth header, but
            # the base class media pipeline should handle that via
            # the session).
            media_urls.append(f"{self._base_url}/api/v4/files/{fid}")
